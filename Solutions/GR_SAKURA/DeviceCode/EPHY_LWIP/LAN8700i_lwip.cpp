...
extern void RX63N_EDMAC_lwip_SetLinkSpeed(BOOL speed, BOOL fullduplex);
...
static BOOL PhyReset(UINT32 addr)
{
    UINT32 i;
    UINT32 data;
    if (!RX63N_EDMAC_lwip_WritePhy(addr, PHY_REG_BMCR, BMCR_RESET, MII_WR_TOUT))
        return (FALSE);
    for (i = 0; i < MII_WR_TOUT; i++) {
        if (!RX63N_EDMAC_lwip_ReadPhy(addr, PHY_REG_BMCR, &data, MII_RD_TOUT))
            return( FALSE );
        if (!(data & BMCR_RESET))
            return true;
    }
...
static BOOL PhyVerifyId(UINT32 addr)
{
    ...
    if (!RX63N_EDMAC_lwip_ReadPhy(addr, PHY_REG_IDR1, &id1, MII_RD_TOUT))
        return false;
#if defined(DEBUG_PHY_LCD)
    dbg_printf("PHY: ReadID OK %04x\r\n", id1);
#endif
    if (!RX63N_EDMAC_lwip_ReadPhy(addr, PHY_REG_IDR2, &id2, MII_RD_TOUT))
        return false;
#if defined(DEBUG_PHY_LCD)
    dbg_printf("PHY: ReadID OK %04x\r\n", id2);
#endif
...
static BOOL PhyAutoNegotiate(UINT32 addr)
{
    ...
    if (!RX63N_EDMAC_lwip_WritePhy(addr, PHY_REG_BMCR, PHY_AUTO_NEG, MII_WR_TOUT))
        return false;
    for (i = 0; i < 10; i++) {
        HAL_Time_Sleep_MicroSeconds(1000000);
        RX63N_EDMAC_lwip_ReadPhy(addr, PHY_REG_BMSR, &data, MII_RD_TOUT);
...
static BOOL PhySetLinkSpeed (UINT32 addr)
{
...
#endif
        if (!RX63N_EDMAC_lwip_ReadPhy(addr, PHY_REG_BMSR, &data, MII_RD_TOUT))
            return false;
    ...
    RX63N_EDMAC_lwip_SetLinkSpeed(mbit_100, full_duplex);
    return true;
}
...
BOOL ENET_PHY_lwip_get_link_status(void)
{
    ...
    if (RX63N_EDMAC_lwip_ReadPhy(phy_addr, PHY_REG_STS, &data, MII_RD_TOUT))
