...
eInitialize_initializeBridge(CELLIDX idx, mrb_state* mrb, struct RClass * TECS)
{
    ...
    mrb_define_method( mrb, rc, "initialize", MrubyBridge_tsButton_initialize, ARGS_REQ(1) );
    MRB_SET_INSTANCE_TT(rc, MRB_TT_DATA);
	mrb_define_method( mrb, rc, "isPressed", MrubyBridge_tsButton_isPressed, ARGS_REQ( 1 ) );
}
