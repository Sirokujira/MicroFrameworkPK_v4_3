...
eInitialize_initializeBridge(CELLIDX idx, mrb_state* mrb, struct RClass * TECS)
{
    ...
    mrb_define_method( mrb, rc, "initialize", MrubyBridge_tsLED_initialize, ARGS_REQ(1) );
    MRB_SET_INSTANCE_TT(rc, MRB_TT_DATA);
	mrb_define_method( mrb, rc, "setColor", MrubyBridge_tsLED_setColor, ARGS_REQ( 1 ) );
	mrb_define_method( mrb, rc, "off", MrubyBridge_tsLED_off, ARGS_NONE() );
}
