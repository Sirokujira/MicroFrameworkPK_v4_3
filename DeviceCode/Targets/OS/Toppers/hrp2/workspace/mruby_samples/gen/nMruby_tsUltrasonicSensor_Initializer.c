...
eInitialize_initializeBridge(CELLIDX idx, mrb_state* mrb, struct RClass * TECS)
{
    ...
    mrb_define_method( mrb, rc, "initialize", MrubyBridge_tsUltrasonicSensor_initialize, ARGS_REQ(1) );
    MRB_SET_INSTANCE_TT(rc, MRB_TT_DATA);
	mrb_define_method( mrb, rc, "getDistance", MrubyBridge_tsUltrasonicSensor_getDistance, ARGS_NONE() );
	mrb_define_method( mrb, rc, "listen", MrubyBridge_tsUltrasonicSensor_listen, ARGS_NONE() );
	mrb_define_method( mrb, rc, "initializePort", MrubyBridge_tsUltrasonicSensor_initializePort, ARGS_NONE() );
}
