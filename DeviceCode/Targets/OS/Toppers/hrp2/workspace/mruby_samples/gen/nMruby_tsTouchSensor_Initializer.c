...
eInitialize_initializeBridge(CELLIDX idx, mrb_state* mrb, struct RClass * TECS)
{
    ...
    mrb_define_method( mrb, rc, "initialize", MrubyBridge_tsTouchSensor_initialize, ARGS_REQ(1) );
    MRB_SET_INSTANCE_TT(rc, MRB_TT_DATA);
	mrb_define_method( mrb, rc, "isPressed", MrubyBridge_tsTouchSensor_isPressed, ARGS_NONE() );
	mrb_define_method( mrb, rc, "initializePort", MrubyBridge_tsTouchSensor_initializePort, ARGS_NONE() );
}
