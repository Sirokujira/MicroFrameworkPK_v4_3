...
eInitialize_initializeBridge(CELLIDX idx, mrb_state* mrb, struct RClass * TECS)
{
    ...
    mrb_define_method( mrb, rc, "initialize", MrubyBridge_tsBalancer_initialize, ARGS_REQ(1) );
    MRB_SET_INSTANCE_TT(rc, MRB_TT_DATA);
	mrb_define_method( mrb, rc, "control", MrubyBridge_tsBalancer_control, ARGS_REQ( 9 ) );
	mrb_define_method( mrb, rc, "init", MrubyBridge_tsBalancer_init, ARGS_NONE() );
}
