...
extern int linux_vsnprintf(char *buffer, size_t size, char *format, va_list arg );

// Global datas
void *glinux_CustomHeap;
void *glinux_Heap;
jmp_buf glinux_ResetBuffer;


int linux_USART_Write( int ComPortNum, const char* Data, size_t size )
{
	size_t result = fwrite(Data, size, 1, stdout);
	return (result > 0 ? size : 0);
}

int linux_USART_Read( int ComPortNum, char* Data, size_t size )
{
...
void linux_debug_printf( const char* format, ... )
{
    char    buffer[256];
    va_list arg_ptr;
	...
	linux_vsnprintf( buffer, sizeof(buffer)-1, (char *) format, arg_ptr );
...
// Arithmetics
//
double linux_ceil(double x)
{
    return(ceil(x));
}

double linux_pow(double x, double y)
{
	return(pow(x, y));
}

double linux_floor(double x)
{
	return(floor(x));}


double linux_fmod(double x, double y)
{	return(fmod(x, y));}
...
long long linux_Time_CurrentTicks();
...
{
    long long now = linux_Time_CurrentTicks();
    master.checked = now;
...
void linux_Time_SetCompare(unsigned long long CompareValue)
{
	long long estimate = CompareValue - linux_Time_CurrentTicks(); // usecs
...
void linux_Time_Sleep_MicroSeconds(unsigned long uSec)
{
	if (LINUXDBG > 1)
...
void linux_CPU_Change_State(int status)
{
	switch(status)
	{
	case RESTART:
...
void linux_CPU_Sleep(void)
{
	pause();
}

void linux_CPU_Restart(void)
{
	longjmp(glinux_ResetBuffer, RESTART);
	/* NOTREACHED */
}

void linux_CPU_Shutdown(void)
{
	longjmp(glinux_ResetBuffer, SHUTDOWN);
	/* NOTREACHED */
}

int linux_InterruptsAreEnabled(void)
{
	return(!masked);
}

void linux_InterruptEnable(void)
{
	if (!linux_InterruptsAreEnabled())
	{
		if (irql == 0)
...
void linux_InterruptDisable(void)
{
	if (linux_InterruptsAreEnabled())
	{
		if (irql == 0)
...
// Memories
//
void*  linux_allocHeap(void)         { return  malloc( (uint32_t) LINUX_HEAP_SIZE ); }
void*  linux_allocCustomHeap(void)   { return  malloc( (uint32_t) LINUX_CUSTOM_HEAP_SIZE ); }
size_t linux_HeapSize(void)          { return  LINUX_HEAP_SIZE; }
size_t linux_CustomHeapSize(void)    { return  LINUX_CUSTOM_HEAP_SIZE; }
void   linux_freeHeap ( void*  ptr ) {         free ( ptr ); }

///////////////
//
// Assembly IOs
//
int linux_loadAssembly(char *name, char *addr)
{
	int fdi;
    char *p;
	size_t length;
...
// Load options
//
int linux_opt_ticks;        //'T'
int linux_opt_ticks_number = 20;
int linux_opt_wait;         //'W'
int linux_opt_wait_ms = 1;
int linux_opt_assert = 1;   //'E' or 'X'

int linux_load_options(int ac, char **av)
{
    int opt;
	...
	while((opt = getopt(ac, av, "dDeEf:F:t:T:w:W:xXzZ")) != -1)
    {
        ...
        case 'T':
            ...
            if (isdigit(*optarg))
            {
                linux_opt_ticks_number = atoi(optarg);
            }
        ...
        case 'W':
            ...
            if (linux_opt_wait_ms == 0)
                linux_opt_wait_ms = linux_opt_wait * 100; // msec
            break;
    ...
    {
        linux_assembly = av[optind];
    }

    //
    // linux_opt_ticks == 1,2,3,...
    // linux_opt_ticks_number == 0~100000...
    //
    if (linux_opt_ticks_number > 0)
    {
        // direct number setting of msec
        period = linux_opt_ticks_number * 1000;
    }
    else if (linux_opt_ticks > 0)
    {
        // 100 msec base setting
        period = linux_opt_ticks * 100 * 1000;
    }
