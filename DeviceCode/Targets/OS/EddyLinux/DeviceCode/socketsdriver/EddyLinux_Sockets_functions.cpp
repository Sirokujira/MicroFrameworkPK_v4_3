...
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LINUX_SOCKETS_Driver::UpdateAdapterConfiguration(interfaceIndex, updateFlags, config);
}

HRESULT HAL_SOCK_CONFIGURATION_LoadWirelessConfiguration( UINT32 interfaceIndex, SOCK_WirelessConfiguration* wirelessConfig )
{
    NATIVE_PROFILE_PAL_NETWORK();
    return LINUX_SOCKETS_Driver::LoadWirelessConfiguration(interfaceIndex, wirelessConfig);
}
...
void HAL_SOCK_SignalSocketThread()
{
	...
	if (linux_opt_wait_ms > 0) // added DEBUG DEBUG DEBUG
	{
		ENABLE_INTERRUPTS();

		Events_WaitForEvents( 0, linux_opt_wait_ms); // added DEBUG DEBUG DEBUG
