...
RX62N_TIME_Driver g_RX62N_TIME_Driver;

void HAL_Time_SetCompare(UINT64 CompareValue)
{
    RX62N_TIME_Driver::SetCompareValue(CompareValue);
}
...
{
    RX62N_TIME_Driver::Sleep_uSec(uSec);
}
...
{
    RX62N_TIME_Driver::Sleep_uSec_Loop(uSec);
}
...
BOOL HAL_Time_Initialize()
{
    return RX62N_TIME_Driver::Initialize();
}
...
BOOL HAL_Time_Uninitialize()
{
    return RX62N_TIME_Driver::Uninitialize();
}

UINT64 HAL_Time_CurrentTicks()
{
    return RX62N_TIME_Driver::CounterValue();
}
...
{
    return RX62N_TIME_Driver::TicksToTime(Ticks);
}

INT64 HAL_Time_CurrentTime()
{
    return RX62N_TIME_Driver::CurrentTime();
}
...
BOOL RX62N_TIME_Driver::Initialize()
{
    g_RX62N_TIME_Driver.m_lastRead    = 0;
    g_RX62N_TIME_Driver.m_nextCompare = (UINT64)RX62N_TIMER_Driver::c_MaxTimerValue;

    if (!RX62N_TIMER_Driver::Initialize(RX62N_TIMER_Driver::c_Timer_System, ISR_Overflow, NULL))
        return FALSE;
    if (!RX62N_TIMER_Driver::Initialize(RX62N_TIMER_Driver::c_Timer_Compare, ISR_Compare, NULL))
        return FALSE;
    return TRUE;
}

BOOL RX62N_TIME_Driver::Uninitialize()
{
    if (!RX62N_TIMER_Driver::Uninitialize(RX62N_TIMER_Driver::c_Timer_System))
        return FALSE;
    if (!RX62N_TIMER_Driver::Uninitialize(RX62N_TIMER_Driver::c_Timer_Compare))
        return FALSE;
    return TRUE;
}

UINT64 RX62N_TIME_Driver::CounterValue()
{
    return g_RX62N_TIME_Driver.m_lastRead + (UINT64)RX62N_TIMER_Driver::ReadCounter(RX62N_TIMER_Driver::c_Timer_System);
}

void RX62N_TIME_Driver::SetCompareValue(UINT64 CompareValue)
{
    GLOBAL_LOCK(irq);

    g_RX62N_TIME_Driver.m_nextCompare = CompareValue;
    UINT64 cntr = CounterValue();
    if (cntr > CompareValue) {
        RX62N_TIMER_Driver::ForceInterrupt(RX62N_TIMER_Driver::c_Timer_Compare);
    } else {
        UINT64 diff = CompareValue - cntr;
        if (diff > RX62N_TIMER_Driver::c_MaxTimerValue) {
            diff = RX62N_TIMER_Driver::c_MaxTimerValue;
        }
        RX62N_TIMER_Driver::SetCompare(RX62N_TIMER_Driver::c_Timer_Compare, (UINT16)diff);
...
void RX62N_TIME_Driver::ISR_Overflow(void* Param)
{
    //GLOBAL_LOCK(irq);
    g_RX62N_TIME_Driver.m_lastRead += (UINT64)RX62N_TIMER_Driver::c_MaxTimerValue;
}

void RX62N_TIME_Driver::ISR_Compare(void* Param)
{
    if (CounterValue() >= g_RX62N_TIME_Driver.m_nextCompare) {
        ...
        //
        SetCompareValue(g_RX62N_TIME_Driver.m_nextCompare);
    }
}

INT64 RX62N_TIME_Driver::TicksToTime(UINT64 Ticks)
{
    return CPU_TicksToTime(Ticks);
}

INT64 RX62N_TIME_Driver::CurrentTime()
{
...
#define RX62N_SLEEP_USEC_FIXED_OVERHEAD_CLOCKS 3
...
//void  RX62N_TIME_Driver::Sleep_uSec(UINT32 uSec) __attribute__ ((section ("SectionForFlashOperations")));

void __attribute__ ((section ("SectionForFlashOperations"))) RX62N_TIME_Driver::Sleep_uSec(UINT32 uSec)
{
    GLOBAL_LOCK(irq);
    ...
    if (maxDiff <= RX62N_SLEEP_USEC_FIXED_OVERHEAD_CLOCKS)
        maxDiff  = RX62N_SLEEP_USEC_FIXED_OVERHEAD_CLOCKS;
    else
        maxDiff -= RX62N_SLEEP_USEC_FIXED_OVERHEAD_CLOCKS;

    while((CounterValue() - value) <= maxDiff);}

//void RX62N_TIME_Driver::Sleep_uSec_Loop(UINT32 uSec) __attribute__ ((section ("SectionForFlashOperations")));

void __attribute__ ((section ("SectionForFlashOperations"))) RX62N_TIME_Driver::Sleep_uSec_Loop(UINT32 uSec)
{
