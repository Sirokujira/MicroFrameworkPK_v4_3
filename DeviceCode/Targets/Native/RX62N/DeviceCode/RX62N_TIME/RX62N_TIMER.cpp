...
RX62N_TIMER_Driver g_RX62N_TIMER_Driver;

BOOL RX62N_TIMER_Driver::Initialize(UINT32 Timer, HAL_CALLBACK_FPN ISR, void* ISR_Param)
{
    ASSERT(Timer < c_MaxTimers);
    GLOBAL_LOCK(irq);
    TimerISR& tmrISR = g_RX62N_TIMER_Driver.m_timers[Timer];
...
BOOL RX62N_TIMER_Driver::Uninitialize(UINT32 Timer)
{
    GLOBAL_LOCK(irq);
    TimerISR& tmrISR = g_RX62N_TIMER_Driver.m_timers[Timer];
    if (tmrISR.Configured)
...
UINT16 RX62N_TIMER_Driver::ReadCounter(UINT32 Timer)
{
    if (Timer == 0) {
...
void RX62N_TIMER_Driver::ForceInterrupt(UINT32 Timer)
{
    if (Timer == 0) {
...
void RX62N_TIMER_Driver::SetCompare(UINT32 Timer, UINT16 Compare)
{
    if (Timer == 0) {
        CMT0.CMCNT = (UINT16)0;
...
static void RX62N_TIMER_ISR(UINT32 Timer)
{
    g_RX62N_TIMER_Driver.m_timers[Timer].ISR(g_RX62N_TIMER_Driver.m_timers[Timer].Arg);
}

#ifdef __cplusplus
extern "C" {
#endif
void __attribute__ ((interrupt)) INT_Excep_CMTU0_CMI0(void) { RX62N_TIMER_ISR(0); }
void __attribute__ ((interrupt)) INT_Excep_CMTU0_CMI1(void) { RX62N_TIMER_ISR(1); }
#ifdef __cplusplus
}
#endif
