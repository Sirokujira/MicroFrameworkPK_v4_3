...
// EP2  Bulk Out
//*********************************************************************************
// CQ_FRK_RX62N Board information
//*********************************************************************************
// VBUS         P16
// USB_DM       47pin
// USB_DP       48pin
...
USB_CONTROLLER_STATE g_RX62N_USB_ControllerState;
...
static INT8 RX62N_USB_Pipe[USB_MAX_EP];
static INT8 RX62N_USB_Zero = 0;
static INT8 RX62N_USB_FifoType[USB_MAX_PIPE];
...
{
    (volatile UINT8 *)&USB0.CFIFO.BYTE.L,
    (volatile UINT8 *)&USB0.D0FIFO.BYTE.L,
    (volatile UINT8 *)&USB0.D1FIFO.BYTE.L
};
#else
...
static void RX62N_USB_Pipe_Initialize(void)
{
    UINT32 i;
    for (i = 0; i < USB_MAX_PIPE; i++) {
        RX62N_USB_FifoType[i] = -1;
    }
    RX62N_USB_FifoType[0] = FIFO_CFIFO;
    RX62N_USB_FifoType[1] = FIFO_D1FIFO;
    RX62N_USB_FifoType[2] = FIFO_CFIFO;
    RX62N_USB_FifoType[3] = FIFO_CFIFO;
    RX62N_USB_FifoType[4] = FIFO_CFIFO;
    RX62N_USB_FifoType[5] = FIFO_CFIFO;
...
void RX62N_USB_Interrupt_Enable(void)
{
    ICU.IER[0x04].BIT.IEN6 = 1;     // USBI0
    ICU.IPR[0x0e].BIT.IPR = 0x0e;   // USBI0 Priority
}

void RX62N_USB_Pullup_Connect(void)
{
    USB0.SYSCFG.BIT.DPRPU = 1;
}

void RX62N_USB_Pullup_Disconnect(void)
{
    USB0.SYSCFG.BIT.DPRPU = 0;
}

void RX62N_USB0_Initialize(void)
{
    MSTP(USB0) = 0;
    PORT1.DDR.BIT.B4 = 1;
    PORT1.ICR.BIT.B6 = 1;
...
void RX62N_USB_Interrupt_Initialize(void)
{
...
void RX62N_USB_Interrupt_Uninitialize(void)
{
    //USB0.SOFCFG.WORD = 0x20;
...
void RX62N_USB_HW_Initialize(void)
{
    RX62N_USB0_Initialize();
    USB0.SYSCFG.BIT.SCKE = 1;
    USB0.SYSCFG.BIT.USBE = 1;
    USB0.SYSCFG.BIT.DCFM = 0;
    USB0.SYSCFG.BIT.DPRPU = 1;
    RX62N_USB_Pipe_Initialize();
    RX62N_USB_Interrupt_Initialize();
    RX62N_USB_Interrupt_Enable();
}

void RX62N_USB_EP_Initialize(UINT32 pipe, UINT32 ep, UINT32 dir, UINT32 attr, UINT32 size)
{
    ClearPipeFIFO(pipe);
    volatile PIPECTR *pipectr = GetPipeCtr(pipe);
...
{
    USB_CONTROLLER_STATE *State = &g_RX62N_USB_ControllerState;
    ...
    while (USB_NextEndpoint(State, pep, pif)) {
        ...
        RX62N_USB_EP_Initialize((UINT32)RX62N_USB_Pipe[endpoint],
                (UINT32)endpoint, (UINT32)dir, pep->bmAttributes, pep->wMaxPacketSize);
    }
}

void RX62N_USB_EP_Reset(void)
{
    USB_DEBUG1("Ep\r\n");
    UINT32 i;
    INT32 pipeno;
    for (i = 0; i < USB_MAX_EP; i++) {
        pipeno = RX62N_USB_Pipe[i];
        if (pipeno >= 0)
            USB_Stall_Clear((UINT32)pipeno);
    }
}

void RX62N_USB_Reset(void)
{
    USB_DEBUG1("Rt\r\n");
    UINT32 i;
    INT32 pipeno;
...
void Pipe_Read(UINT32 pipe, volatile UINT8 *buf, UINT32 size, volatile UINT32 *count)
{
    UINT8 dummy;
    UINT32 len;
    UINT8 type = RX62N_USB_FifoType[pipe];
    GLOBAL_LOCK(irq);
...
void HandleSetupCmd(void)
{
    ...
    USB_SETUP_PACKET *st = (USB_SETUP_PACKET *)&USB_EP0Buffer[0];
    st->bmRequestType = (UINT8)(USB0.USBREQ.WORD & 0xff);
    st->bRequest = (UINT8)(USB0.USBREQ.WORD >> 8);
    st->wValue = USB0.USBVAL;
    st->wIndex = USB0.USBINDX;
    st->wLength = USB0.USBLENG;
    USB_DEBUG4("U%02x:%04x:%04x", st->bRequest, st->wValue, st->wLength);
...
void RX62N_USB_Send(USB_CONTROLLER_STATE *State, UINT32 endpoint)
{
    GLOBAL_LOCK(irq);
    USB_PACKET64* Packet64 = USB_TxDequeue(State, endpoint, TRUE);
    UINT32 pipeno = RX62N_USB_Pipe[endpoint];
...
void RX62N_USB_Receive(USB_CONTROLLER_STATE *State, UINT32 endpoint)
{
    GLOBAL_LOCK(irq);
    BOOL full;
    UINT32 pipeno = RX62N_USB_Pipe[endpoint];
...
void HandleBEMP(void)
{
    ...
    if (1 == USB0.BEMPSTS.BIT.PIPE0BENP) {
        USB_DEBUG1("Be0");
        ...
        for (UINT32 endpoint = 1; endpoint < USB_MAX_EP; endpoint++) {
            INT8 pipeno = RX62N_USB_Pipe[endpoint];
            if (pipeno >= 0) {
		...
		for (UINT32 endpoint = 1; endpoint < USB_MAX_EP; endpoint++) {
			...
			if (pipeno >= 0) {
			    ...
			    if ((brdysts & mask) && (brdyenb & mask)) {
			        ...
			        if (USB0.PIPECFG.BIT.DIR == 0) {
			            RX62N_USB_Receive(State, endpoint);  // OUT
			        } else {
			            RX62N_USB_Send(State, endpoint);     // IN
			        }
			    }
			}
		}
...
USB_CONTROLLER_STATE * CPU_USB_GetState(int Controller)
{
    if (Controller != 0)
        return NULL;
    return (USB_CONTROLLER_STATE *)&g_RX62N_USB_ControllerState;
}
...
HRESULT CPU_USB_Initialize(int Controller)
{
    ...
    while (USB_NextEndpoint(State, pep, pif)) {
        ...
        case USB_ENDPOINT_ATTRIBUTE_BULK:
            RX62N_USB_Pipe[endpoint] = pipe_bulk;
            if (pep->bEndpointAddress & 0x80) {
                RX62N_USB_EP_Initialize((UINT32)pipe_bulk++,
        ...
        case USB_ENDPOINT_ATTRIBUTE_INTERRUPT:
            RX62N_USB_Pipe[endpoint] = pipe_interrupt;
            if (pep->bEndpointAddress & 0x80) {
                RX62N_USB_EP_Initialize(pipe_interrupt++,
...
HRESULT CPU_USB_Uninitialize(int Controller)
{
    USB_CONTROLLER_STATE *State = &g_RX62N_USB_ControllerState;
    GLOBAL_LOCK(irq);
    memset(&g_RX62N_USB_ControllerState, 0, sizeof(g_RX62N_USB_ControllerState));
    CPU_USB_ProtectPins(Controller, TRUE);
    USB_EP_Type = 0;
    State->DeviceState = USB_DEVICE_STATE_DETACHED;
    RX62N_USB_Interrupt_Uninitialize();
    USB_DEBUG1("*U*");
    return S_OK;
}
