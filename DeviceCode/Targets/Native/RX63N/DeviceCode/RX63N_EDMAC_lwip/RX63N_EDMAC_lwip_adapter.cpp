...
struct netif g_RX63N_EDMAC_NetIF;
...
void RX63N_EDMAC_status_callback(struct netif *netif)
{
...
#define NETIF_FLAG_ETHERNET 0
#endif

err_t RX63N_EDMAC_ethhw_init(struct netif *myNetIf)
{ 
    myNetIf->flags = NETIF_FLAG_IGMP | NETIF_FLAG_ETHARP | NETIF_FLAG_ETHERNET; // Added in 4.3.1
    myNetIf->mtu = RX63N_EDMAC_MAX_FRAME_SIZE;
    myNetIf->output = etharp_output;
    myNetIf->linkoutput = RX63N_EDMAC_LWIP_xmit;
    myNetIf->status_callback = RX63N_EDMAC_status_callback;
    RX63N_EDMAC_LWIP_open(myNetIf);
    return (err_t)ERR_OK;
}
...
void lwip_network_uptime_completion(void *arg)
{
    ...
    BOOL status = RX63N_EDMAC_LWIP_GetLinkStatus();
    if (status != LwipNetworkStatus) {
        ...
        if (status) {
            ...
            RX63N_EDMAC_LWIP_AutoNegotiate();
            netif_set_up(pNetIf);
...
{
#ifndef HW_INTERRUPT
    InterruptTaskCompletion.InitializeForISR((HAL_CALLBACK_FPN)RX63N_EDMAC_LWIP_recv, pNetIf);
...
{
    return RX63N_EDMAC_LWIP_Driver::Bind();
}

int  Network_Interface_Open(int index)
{
    return RX63N_EDMAC_LWIP_Driver::Open(index);
}

BOOL Network_Interface_Close(int index)
{
    return RX63N_EDMAC_LWIP_Driver::Close();
}

int RX63N_EDMAC_LWIP_Driver::Open(int index)
{
    ...
    len = g_RX63N_EDMAC_NetIF.hwaddr_len;
    if (len == 0 || iface->macAddressLen < len) {
        len = iface->macAddressLen;
        g_RX63N_EDMAC_NetIF.hwaddr_len = len;
    }
    memcpy(g_RX63N_EDMAC_NetIF.hwaddr, iface->macAddressBuffer, len);
    ipaddr.addr     = iface->ipaddr;
    gateway.addr    = iface->gateway;
    subnetmask.addr = iface->subnetmask;

    pNetIF = netif_add(&g_RX63N_EDMAC_NetIF, &ipaddr, &subnetmask, &gateway, NULL, RX63N_EDMAC_ethhw_init, ethernet_input);

    netif_set_default(pNetIF);
    LwipNetworkStatus = RX63N_EDMAC_LWIP_GetLinkStatus();
    if (LwipNetworkStatus)
        netif_set_up(pNetIF);
    InitCompletions(pNetIF);
    return g_RX63N_EDMAC_NetIF.num;
}

BOOL RX63N_EDMAC_LWIP_Driver::Close(void)
{
    int retVal = -1;
...
#endif
    LwipNetworkStatus = 0;
    netif_set_down(&g_RX63N_EDMAC_NetIF);
    netif_remove(&g_RX63N_EDMAC_NetIF);
    RX63N_EDMAC_LWIP_close(&g_RX63N_EDMAC_NetIF);
    memset(&g_RX63N_EDMAC_NetIF, 0, sizeof g_RX63N_EDMAC_NetIF);
    return true;
}

BOOL RX63N_EDMAC_LWIP_Driver::Bind(void)
{
    return true;
}

BOOL RX63N_EMAC_LWIP_GetLinkStatus(void)
{
    return true;
}
